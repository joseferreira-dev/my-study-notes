...
### Links para Fragmentos da Página
...
## Aparência dos Links: Estados e Padrões Visuais
...
## Destino do Link: Atributo `target`
...
1. **Estilizar um Link (`<a>`) para Parecer um Botão (Melhor Prática)**: A forma correta e mais acessível é usar a tag `<a>` e aplicar estilos CSS para que ela tenha a aparência de um botão. Isso mantém a semântica correta: é um link de navegação.
